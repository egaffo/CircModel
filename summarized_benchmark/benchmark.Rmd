...
deseq2lrt_run <- function(countData, colData, design, contrast) {
  ...
  results(dds, independentFiltering = F)
}
...
edgeR_run <- function(countData, group, design) {
  ...
  y <- calcNormFactors(y) # method = "TMMwsp" for zero counts?
  des <- model.matrix(design)
  y <- estimateDisp(y, des) # min.row.sum = 5 default. Change?
  fit <- glmFit(y, des)
  glmLRT(fit, coef=2)
}
...
edgeRzw_run <- function(countData, group, design, weights) {
  ...
  y <- calcNormFactors(y)
  des <- model.matrix(design)
  y <- estimateGLMRobustDisp(y, des) # estimateDisp(y, des)?
  fit <- glmFit(y, des) #glmQLFit?
  glmLRT(fit, coef = 2)
}
...
edgeRql_run <- function(countData, group, design, weights) {
  ...
  y <- calcNormFactors(y)
  des <- model.matrix(design)
  y <- estimateDisp(y = y, design = des, robust = TRUE)
  fit <- glmQLFit(y, design = des, robust = TRUE)
...
voomQn_run <- function(countData, group, design) {
  ...
  limma::eBayes(limma::lmFit(voom.data, design = des))
}
...
voomSimple_run <- function(countData, group, design) {
  ...
  des <- model.matrix(design)
  limma::eBayes(limma::lmFit(limma::voom(counts = countData, 
                                         design = des), 
                             des), 
                robust = TRUE)
}
...
voomLmFit_run <- function(countData, group, design) {
  ...
  limma::eBayes(edgeR::voomLmFit(counts = countData, 
                                 design = des, 
                                 # block = NULL, 
                                 # prior.weights = NULL,
                                 sample.weights = TRUE,
...
bd <- bd %>%
  addMethod(label = "DESeq2_Dt_WaT", ## DESeq2, default parameters, Wald test
            func = deseq2_run, post = deseq2_pv,
  ...
  addMethod(label = "DESeq2_Dt_LRT", ## DESeq2, default parameters, likelihood ratio test
            func = deseq2lrt_run, post = deseq2_pv,
            meta = list(pkg_name = "DESeq2", pkg_vers = as.character(packageVersion("DESeq2"))),
            params = rlang::quos(countData = cntdat,
                                 colData = coldat, 
                                 design = ~condition,
  ...
  addMethod(label = "DESeq2_Lc_LRT", ## DESeq2, parameters for low counts, likelihood ratio test
            func = deseq2lc_run, post = deseq2_pv,
            meta = list(pkg_name = "DESeq2", pkg_vers = as.character(packageVersion("DESeq2"))),
            params = rlang::quos(countData = cntdat,
                                 colData = coldat, 
                                 design = ~condition,
  ...
  addMethod(label = "DESeq2_Zi_LRT", ## DESeq2, parameters for single-cell data, likelihood ratio test
            func = deseq2zi_run, post = deseq2_pv, 
            meta = list(pkg_name = "DESeq2", pkg_vers = as.character(packageVersion("DESeq2"))),
            params = rlang::quos(countData = cntdat,
                                 colData = coldat, 
                                 design = ~condition,
  ...
  addMethod(label = "DESeq2_Sc_LRT", ## DESeq2, parameters for single-cell data and scran::computeSumFactors, likelihood ratio test
            func = deseq2sc_run, post = deseq2_pv,
  ...
  addMethod(label = "DESeq2_GP_LRT", ## DESeq2, Gamma-Poisson, likelihood ratio test
            func = deseq2gpLrt_run, post = deseq2_pv, 
  ...
  addMethod(label = "DESeq2_ZW_LRT", ## DESeq2, ZinbWave, likelihood ratio test
            func = deseq2zw_run, post = deseq2_pv, 
  ...
  addMethod(label = "DESeq2_BP_WaT", ## DESeq2, betaPrior=T, Wald test
            func = deseq2bp_run, post = deseq2_pv,
            meta = list(pkg_name = "DESeq2", pkg_vers = as.character(packageVersion("DESeq2"))),
            params = rlang::quos(countData = cntdat,
                                 colData = coldat, 
                                 design = ~condition,
  ...
  addMethod(label = "edgeR_ZW_LRT", ## edgeR, ZinbWave, likelihood ratio test 
            func = edgeRzw_run, post = edgeR_pv,
...
datasetList <- 
  bplapply(datasetList, 
           function(x, compute_weights, zwcpus) {
             ...
             ## remove rows with at least 2 samples expressed
...
sbL <- bplapply(datasetList, 
                function(x, bd) { SummarizedBenchmark::buildBench(bd, data = x, truthCols = "status") },
                bd = bd,
...
sbL <- bplapply(sbL, function(x) {
  SummarizedBenchmark::addPerformanceMetric(x, 
...
sbL <- bplapply(sbL, function(x, alphas) {
