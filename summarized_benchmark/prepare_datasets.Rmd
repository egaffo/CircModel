...
simulated_datasets_qs <- "../semi_parametric_sim/trimmed_simulated_datasets.qs"
...
# circsKept_dt[!grep("mock", Dataset), Dataset := sub("(_[0-9]{2}$)", "_de\\1", Dataset)]
...
```{r}
## circRNAs common to all replicates
allRepsCircs <- 
  circsKept_dt[, .(nDS = .N), 
               by = .(SetSize, MZP, Purpose, DE.ind, 
                      source.ID)][nDS == 30]


dcast(allRepsCircs[, .N, 
                   by = .(SetSize, MZP, Purpose)],
      SetSize + Purpose ~ MZP, 
      fill = 0)[order(Purpose, SetSize)]
```
...
dcast(allRepsCircs[, .N, 
                   by = .(SetSize, MZP, Purpose, 
                          DE.ind)
                   ][, DE.ind := ifelse(DE.ind, 
                                        "DEC", 
                                        "NotDEC")
                     ][, Tot := sum(N), by = .(SetSize, MZP, Purpose)][], 
      SetSize + MZP + Purpose + Tot ~ DE.ind, 
      value.var = "N", 
      fill = 0)[order(Purpose, MZP, SetSize), ]
```

```{r}
sourceIdList <- 
  split(allRepsCircs[, .(SetSize, MZP, Purpose, source.ID)], 
        by = c("SetSize", "MZP", "Purpose"), 
        flatten = T, keep.by = F)

names(sourceIdList) <- gsub("\\.", "_", names(sourceIdList))
```

```{r}
# sum(datasetList$N03_bulk_de_01$rowData$DE.ind)
```
...
datasetMetaList <-
  lapply(setNames(names(datasetList), nm = names(datasetList)),
         ...
         # circList = sourceIdList,
         dsList = datasetList)


table(sapply(datasetMetaList, function(x) x$nGenes))

# a<-sim_ds_list$N03_bulk$Datasets$detailed.results
# a$densList$`2:199940088-199948472`

## number of DECs
summary(sapply(datasetMetaList, function(x) sum(x$status)))
```

```{r save_datasets}
## save the processed data sets
datasetList_qs <- file.path(outdir, "datasetList.qs")
