...
cols <- c(
  # DEseq
  brewer.pal(n = 9, "YlOrRd")[c(4,5)],
  # Edger
  brewer.pal(n = 9, "GnBu")[c(6,7)],
  # limma
...
```{r}
load("/blackhole/alessia/circzi/checkCircRNAnormalizationdistribution/robustness_glmm/DM1/sensitivityPrecision10_CIRI.RData")
...
conc.witbet <- compute_concordance_withbetw(ps_fitted_list = ps_fitted, maxrank = 500)
conc.betglmm <- compute_concordance_withGLMM(ps_fitted_list = ps_fitted, maxrank = 500)
...
AUC_AOC_between_methods <- suppressWarnings(ddply(concordance_df_summary, ~ method1 + method2, function(x) AucAocFun(cVals = x$concordance[x$rank <= 400], nfeatures = mean(x$nfeatures[x$rank <= 400]), threshold = 400)))
...
conc_df_sub <- conc.betglmm[conc.betglmm$rank == 400 & conc.betglmm$subset == "HeldoutvsTest",]
...
ggplot(conc_df_sub, aes(x = method1, y = concordance, color = method1)) +
    geom_boxplot() +
    coord_flip() +
    scale_x_discrete(limits = unique(conc_df_sub$method1)[rev(ord)]) +
