...
```{r, fig.width=6.69, fig.height=8.85}
tnr_pattern <- "^TNR_0\\.[015]{2}$" 
Alpha_plot_label <- "P-value \U2264 \U003B1"
...
fpr_mat <- 
  as.matrix(data.frame(dcast(data = sbAssay_dt[, .(Dataset = paste(Source_ds, Dataset), 
...
fpr_clusts <- dendsort(hclust(fpr_dists, method = "ward.D2"), 
                       isReverse = T, 
                       type = "min")
```
...
fpr_dendro_wrap_plot <- 
  wrap_plots(wrap_plots(fpr_dendro + theme_void() + 
             ...
             fpr_dendro_ordered_plot + theme(plot.margin = margin(0, 0, 0, 0, "lines"),
                                             axis.ticks.x = element_blank(),
                                             panel.grid.minor.y = element_blank()), 
             plot_stn_clustered + theme(plot.margin = margin(0, 0, 0, 0, "lines"),
                                        axis.text.x = element_blank()),
             colored_method_text + theme_void() + 
                 theme(plot.margin = margin(0, 0, 0, 0, "lines")), 
             ncol = 1, 
             heights = c(1.25, 2.5, 7, 3.5)) &
...
# n_points$y <- 1.7
# 
# ggplot(plot_dt[][, Method.2 := str_wrap(gsub("_", " ", Method), width = 10)], 
#        aes(x = SetSize, y = value, color = Method, group = paste(Method, SetSize))) +
#     geom_hline(data = plot_dt[, .N, by = .(SetSize, variable)],
#                aes(yintercept = as.numeric(variable)), 
#                linetype = "dashed") +
#     geom_boxplot(outlier.size = .5) +
...
#     scale_color_manual(values = method_colors, guide = "none") +
#     scale_y_sqrt(breaks = c(0, 0.01, 0.05, 0.1, .5, 1)) + #, expand = c(0, 0)
#     facet_grid(rows = vars(variable), cols = vars(Method.2),
#                scales = "fixed", 
#                labeller = label_wrap_gen(width = 5, multi_line = T)) +
#     # facet_grid(rows = vars(variable)) + ## x = Method
#     theme_bw() +
#     theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5),
#           panel.grid.major.x = element_blank(), 
...
# dist_base_mat <- (fdr_mat - 0.05)^3
dist_base_mat <- scale(fdr_mat,
                          center = rep_len(0.05, dim(fdr_mat)[2]),
                          scale = T)
# dist_base_mat <- t(apply(X = fdr_mat, MARGIN = 2, FUN = median, na.rm = T)
