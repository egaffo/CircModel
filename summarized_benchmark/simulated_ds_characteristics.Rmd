...
# ggplot(cor_dt, aes(x = DS, y = CorPear)) +
#     geom_boxplot(aes(color = Group), varwidth = F, notch = F) +
#     scale_color_economist() +
#     labs(x = "Data set", y = "Pearson's correlation on BJR") +
#     theme_bw() +
...
```{r, fig.width=4, fig.height=3}
ggplot(rbind(cor_dt, pc_cor_dt[, DS:= "PC"]), aes(x = DS, y = CorPear)) +
    geom_boxplot(aes(color = Group), varwidth = F, notch = F) +
    scale_color_economist() +
    labs(x = "Data set", y = "Pearson's correlation on BJR") +
    theme_bw() +
