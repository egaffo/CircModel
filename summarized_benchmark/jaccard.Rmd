...
n05_halfheat_plot <- 
    ...
    guides(color = guide_colorbar(title.position = "top", title.hjust = .5, 
           ...
           fill  = guide_colorbar(title.position = "top", title.hjust = .5, 
                                  frame.colour = "black")) +
    scale_x_discrete(limits = method_order, position = "top") +
    scale_y_discrete(limits = method_order) +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 90, hjust = 0, color = method_colors[method_order]),
...
method_order <- label(plots_avg_jdn$N05$ddata)$label
