...
#                x.sign <- 1
#                if(x < 0) x.sign <- -1
#                ifelse(x == 0, 0, log2(abs(x)) * x.sign)
#                }
#            )
# }
# 
# inv_signed_log2 <- function(x){
#     sapply(x, 
#            function(x){
#                x.sign <- 1
#                if(x < 0) x.sign <- -1
#                ifelse(x == 0, 0, 2^(abs(x)) * x.sign)
#                }
#            )
# }
# 
# signed_log2_trans <- function() {
#     trans_new(name = "signed_log2", 
#               transform = signed_log2, 
#               inverse = inv_signed_log2)
# }
...
plot_stn_boxes <- 
  ggplot(plot_dt[Val != "Data sets w/TP"], 
         aes(x = Method, y = StN)) +
  geom_hline(data = ~ subset(.x, Datatype == "Measures"), ##  & StN <= 2  & StN >= -2 NB: the output is truncated
             aes(yintercept = 0), 
             linetype = "dashed") +
...
plot_dt <- copy(pval_deind)[SetSize == "N05"][is.na(preds), preds := 1]
...
ggplot(plot_dt, aes(x = Pval.bins, y = Mean * 100)) +
    ...
    facet_wrap(facets = ~ Method, ncol = 5, scales = "free_y") +
    theme_bw() +
...
plot_dt <- plot_dt[, .N, by = .(SetSize, Source_ds, Method, Dataset, Pval.bins, true_val)]

plot_dt[, Frac := N/sum(N), by = .(SetSize, Source_ds, Method, Dataset)]

plot_dt <- plot_dt[, .(Mean = mean(Frac),
                       SE = sqrt(var(Frac) / length(Frac))),
                   by = .(SetSize, Method, Pval.bins, true_val)]

ggplot(plot_dt, aes(x = Pval.bins, y = Mean * 100)) +
    geom_col(aes(fill = factor(true_val)), position = "stack", color = "grey50") +
    # geom_errorbar(aes(ymin = (Mean - SE) * 100, ymax = (Mean + SE) * 100), 
    #               color = "black", width = .75) +
    facet_wrap(facets = ~ Method, ncol = 5, scales = "free_y") +
    scale_fill_manual(name = "True DEC", 
                      values = c("0" = "lightblue", "1" = "lightgoldenrod"), 
                      labels = c("No", "Yes")) +
    guides(fill = guide_legend(ncol = 2, override.aes = list(alpha = 1, size = 2, color = NULL),
                                title.vjust = .5, 
                                title.position = "left")) +
    theme_bw() +
    labs(x = "P values", y = "Mean fraction (%)") +
    theme(legend.position = c(.75, 0), 
          axis.text.x = element_text(angle = 90, vjust = .5, size = 5),
          axis.text.y = element_text(size = 6),
          panel.grid = element_blank(),
          strip.text = element_text(size = 7))
```
...
plot_dt[, tot := sum(N), by = .(SetSize, Source_ds, Method, Dataset)]

ggplot(plot_dt, aes(x = Pval.bins, y = (N/tot)*100)) +
    geom_line(aes(group = paste(Source_ds, Dataset), 
                  color = Source_ds), alpha = 1/10) +
    facet_wrap(facets = ~ Method, ncol = 5, scales = "free_y") +
    scale_color_tableau(name = "Data origin", direction = -1) +
    guides(color = guide_legend(ncol = 2, override.aes = list(alpha = 1, size = 2),
                                title.vjust = .5, 
                                title.position = "left")) +
    theme_bw() +
    labs(x = "P values", y = "Fraction (%)") +
...
```{r, fig.width=8.85, fig.height=6.69}
plot_dt <- copy(pval_deind)[SetSize == "N10"][is.na(preds), preds := 1]
...
plot_dt <- sbAssay_dt[, Seconds := ifelse(grepl("_ZW_", Method), Runtime.2, 
                                          Runtime.1)][, .(MeanSeconds = mean(Seconds)), 
                                                      by = .(SetSize, Method)]
...
cputime_plot <-
    ...
    scale_y_sqrt(breaks = c(0, 0.1, 1, 10, 30, 60, 120, 240, 360, 480)) +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5, 
                                     color = method_colors[levels(plot_dt$Method)]),
          panel.grid.major.x = element_blank(),
