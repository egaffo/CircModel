...
ggplot(plot_dt,
    ...
    geom_label_repel(data = head(plot_dt[CUTOFF <= .1][order(CUTOFF, FDR, TPR, decreasing = T)], 1),
...
```{r}
auROCcs_file <- file.path(output_dir, "FDRvsTPR_AUC.csv")
fwrite(auROCcs, 
       auROCcs_file, 
       sep = "\t", row.names = F)
```

FDR/TPR AUC values have been saved into <a href="`r auROCcs_file`">`r auROCcs_file`</a>. 
...
## AUC values
auROCcs <-
    rbindlist(lapply(split(adj_pval_deind_dt, #[SetSize == "N05" & MZP == "bulk"]
             by = c("SetSize", "MZP", "DsType", "Method")), 
       function(x){
...
```{r}
auROCcs_file <- file.path(output_dir, "ROC_AUC.csv")
fwrite(auROCcs, 
       auROCcs_file, 
       sep = "\t", row.names = F)
```
...
#                            by = .(Dataset, SetSize, MZP, DsType,
#                                   Method = as.character(Method))]
auROCcs <- adj_eval_metrics_dt[order(TPR, FPR), 
                               .(AUC = simple_auc(TPR, FPR)),
                               by = .(Dataset, SetSize, MZP, DsType,
                                      Method = as.character(Method))]
...
ggplot(plot_dt, 
  ...
  scale_y_log10("Runtime (mm:ss)", 
                ...
                # labels = c("0.1s", "1s", "10s", "1m", "10m", "1h")) +
...
ggplot(plot_dt, aes(x = F1, y = mTime, color = Method, shape = Alpha)) +
  geom_point() +
  facet_grid(cols = vars(SetSize),
             rows = vars(MZP, Alpha), 
             scales = "free") +
