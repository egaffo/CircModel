...
output: 
  html_document: 
    ...
    code_folding: hide
params:
  output_dir: "IPF_evaluations"
  simulated_datasets_qs: "IPF_prep_ds/datasetList.qs"
  sumBench_perf_metrics_qs: "IPF_performance/sumBench_perf_metrics.qs"
  sice: FALSE
editor_options: 
...
ggplot(plot_dt, aes(x = Method, y = 1 - value)) +
    geom_hline(data = unique(plot_dt[, c("SetSize", "Alpha"), with = F]), 
               aes(yintercept = Alpha), 
               linetype = "dashed") +
    geom_boxplot(outlier.shape = NA) +
    geom_jitter(aes(color = Method), width = .1, size = .6) +
    ylab("Average FPR") +
...
ggplot(plot_dt[Alpha == 0.05 & SetSize == "N05"], 
    ...
    geom_hline(data = unique(plot_dt[, c("SetSize", "Alpha"), 
               ...
               linetype = "dashed") +
    geom_boxplot(outlier.shape = NA) +
    geom_jitter(aes(color = Method), width = .1) +
    ylab("FPR") +
    scale_color_discrete(guide = "none") +
    scale_shape_discrete(Alpha_plot_label, labels = alpha_targets) +
    # facet_grid(cols = vars(SetSize), 
...
ggplot(plot_dt[Alpha == 0.05 & SetSize == "N05"], 
    ...
    labs(y = "FPR (P < 0.05)", x = NULL, title = "5 samples per group") +
    scale_color_discrete(guide = "none") +
...
ggplot(plot_dt, aes(x = nCircRNAs, y = F1, 
                color = Method)) +
...
ggplot(mean_plot_dt, aes(x = Method, y = AUC)) +
  ...
  geom_pointrange(aes(ymin = AUC - sdAUC, ymax = AUC + sdAUC, fill = Method),
                  # fill = "red", 
                  shape = 21) +
  ylab("Average AUROC (FPR, TPR)") +
  facet_grid(cols = vars(SetSize), 
             scales = "free_y") +
