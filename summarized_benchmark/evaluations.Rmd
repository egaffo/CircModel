...
```{r}
simulated_datasets_qs <- "/sharedfs01/circrna/CircModel/summarized_benchmark/datasetList.qs"
...
cmL <- lapply(sbL, function(x) {
  status_mat <- assays(x)[["status"]] <= 0.1
  status_mat[is.na(status_mat)] <- F
  
  cm <- make_comb_mat(status_mat, min_set_size = 0)
  
  list(overlap_frac = comb_size(cm) / nrow(assays(x)[["status"]]),
       # comb_name(cm),
       cm = cm,
       set_names = set_name(cm))
})
...
setSize <- 
  rbindlist(lapply(cmL, function(x) {
    data.table(Method = names(set_size(x$cm)),
               setSize = set_size(x$cm),
               setSize_frac = set_size(x$cm) / (comb_size(x$cm)[1] / x$overlap_frac[1]))
  }),
  idcol = "DS")
...
plot_common_pred_upset <- function(setsize, mzp, purpose, 
  ...
  cmt <- make_comb_mat(t(mt), min_set_size = 0)
  
  top_boxplot_mat <- 
    as.matrix(data.frame(dcast(ovlp_frac[Comb %in% comb_name(cmt) &
                                           SetSize == setsize & 
                                           MZP == mzp & 
                                           Purpose == purpose], 
                               DS ~ Comb, 
