...
SPsimSeq_simulate <- 
  function(params_list, ...) {
    ...
    cand_DE_genes <- list(nonnull.genes = params_list$cand_DE_genes,
                          null.genes = rownames(counts_mt)[!rownames(counts_mt) %in% params_list$cand_DE_genes])
    w <- params_list$w
...
min_samples <- floor(ncol(orig_mat) / 4) #3
...
`r Ncircrnas` to be simulated will be selected randomly but choosing `r Ncircrnas / 10` for each expression quantile.  
...
params_list <- 
  list(counts_mt = orig_mat,
       Ncircrnas = Ncircrnas, 
       fracDECs = fracDECs,
       cand_DE_genes = nonnull_circs_names,
