...
load(file = "/blackhole/alessia/circzi/checkCircRNAnormalizationdistribution/robustness_glmm/DM1/sensitivityPrecision10_CIRI.RData")
...
d <- data.frame(heldoutCalls=reshape2::melt(heldMat)$value,
                ...
                algorithm=factor(rep(namesAlgosALL,each=nrow(heldMat)),
                levels=namesAlgosALL))
...
res = t(sapply(1:nreps, function(i) sapply(namesAlgosALL, function(algo) {
