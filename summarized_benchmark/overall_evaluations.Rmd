...
method_types <- 
  data.table(Method = c("circMeta_Dt_PZT", "circMeta_Lc_PZT", "DESeq2_BP_WaT", 
             ...
             Type = c("Bulk", "Bulk", "Bulk", 
                      "Bulk", "Bulk", "Single cell / metagenome", 
                      "Single cell / metagenome", "Single cell / metagenome", "Single cell / metagenome", 
                      "Single cell / metagenome", "Single cell / metagenome", "Bulk", 
                      "Bulk", "Bulk", "Bulk", 
                      "Bulk", "Single cell / metagenome", "Single cell / metagenome", 
                      "Single cell / metagenome", "Bulk", "Single cell / metagenome", 
                      "Single cell / metagenome", "Single cell / metagenome", "Single cell / metagenome", 
                      "Single cell / metagenome", "Bulk", "Bulk", 
                      "Single cell / metagenome", "Bulk", "Single cell / metagenome", 
                      "Single cell / metagenome", "Bulk", "Single cell / metagenome",
                      "Bulk", "Bulk", "Bulk", 
...
# method_types_colors <- sapply(split(method_types_colors.dt, method_types_colors.dt$BaseMethod),
#                               function(x) setNames(x$Color, x$BaseMethod), USE.NAMES = F)
method_types_colors <- unlist(split(method_types_colors.dt$Color, method_types_colors.dt$BaseMethod))
```
...
plot_base_method_tile_wborder <- 
  ...
  scale_color_manual(values = c("Bulk" = "black", 
                                "Single cell / metagenome" = "grey")) +
...
plot_dt <- 
  melt(copy(method_types)[, c("Base", "Parameters", "Test") := tstrsplit(Method, "_")][, .(Method,
                                                                                           BaseMethod,
                                                                                           Parameters,
                                                                                           Test)], 
                id.vars = c("Method"))

plot_base_method_text <-
  ggplot(plot_dt, aes(x = Method, y = variable, fill = value)) +
  # geom_tile(color = "white", width = 0.9, height = 0.9, size = .5) +
  geom_text(aes(label = value), angle = 90, hjust = 0.5, vjust = .5) +
  # coord_cartesian(expand = F) +
  labs(x = NULL, y = NULL) +
  # scale_fill_manual(values = c(method_types_colors, 
  #                              "Bulk" = "black", 
...
## NB: all methods gave results in all the data sets. 
## Plot the numbers for consistency with the other plots below
...
plot_stn <- 
  ...
  labs(x = NULL, y = "Signal-to-noise statistics (DECs / not DECs)") +
  scale_color_discrete(guide = "none") +
  # scale_fill_brewer(name = "Data set", palette = "Set2", direction = -1) +
  scale_fill_tableau(name = "Data set", direction = -1, na.translate = F,
...
wrap_plots((plot_fpr_n05_005 + ylab("FPR (P \U2264 0.05)") +
             theme(strip.background = element_rect(fill = NA), 
                   plot.margin = unit(c(0, 0, 0, 0), "lines"),
                   axis.ticks.x = element_blank(),
...
### N05 FPR + Signal-to-noise + namebox

```{r, fig.height=8.858, fig.width=6.692}
panel_a <- 
  (plot_fpr_n05_005 + ylab("FPR (P \U2264 0.05)") +
             theme(strip.text.x = element_blank(),
                   strip.background = element_rect(fill = NA), 
                   plot.margin = unit(c(0, 0, 0, 0), "lines"),
                   axis.ticks.x = element_blank(),
...
## Avg TPR
ggplot(merge(method_types, 
             plot_dt[SetSize == "N05" & variable == 0.05], 
             by = "Method"),
       aes(x = Method, y = value, color = Method)) +
  geom_boxplot(outlier.size = .5, varwidth = F) + #outlier.shape = NA
  # geom_jitter(width = .1, size = .5) +
  geom_text(data = merge(method_types, 
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5),
        panel.grid.major.x = element_blank(),
        legend.position = "top", panel.spacing.x = unit(.1, "lines"))
```
