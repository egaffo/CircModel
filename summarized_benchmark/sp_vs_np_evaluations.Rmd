...
sbAssay_dt[, `:=`(F1_0.01 = f.beta(1-FDR_0.01_padj, TPR_0.01_padj),
...
value_cols <- 
  c(grep("rejections", colnames(sbAssay_dt), value = T),
    ...
    grep("F1", colnames(sbAssay_dt), value = T))
...
```{r}
mean_ranks <- rbindlist(list(SP_TPR = tpr_mean_rank, 
...
all_meas_ranks <- rbindlist(list(FDR = npsp_fdr_rank[, .(Sim, Source_ds, SetSize, Method, Rank, 
