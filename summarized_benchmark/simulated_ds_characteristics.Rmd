...
ls_bxp <- 
  ggplot(source_ds_dt[circ_id %in% circ_id3s, 
                      .(LibSize = sum(value)), by = .(DS, sample_id)],
         aes(x = "", y = LibSize)) +
...
ggplot(plot_dt, aes(x = `Mean library size`, y = Sparsity, color = DS)) +
    geom_smooth(method = "glm", group = 1, color = "black", linetype = "dashed", se = T) +
    geom_point(aes(size = Features)) +
    geom_label_repel(aes(label = DS), show.legend = F) +
    annotate(geom = "label", x = 20000, y = .25, 
...
ggplot(df.b2, aes(x = PC1, y = PC2, color = Group)) +
    geom_point() +
    ggtitle("Batch2")
```
