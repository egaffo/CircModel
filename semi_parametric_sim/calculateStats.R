...
#
#     obs_stats <-
#         data.table::rbindlist(bplapply(setNames(unique(df$dataset),
#                                                 unique(df$dataset)),
#                                        function(x, ref, df,
#                                                 calculateStats,
#                                                 column,
...
#                                                 kfrac) {
#
#                                            dftmp <- data.frame(data.table::rbindlist(list(df[df$dataset == x, ], ref),
#                                                                                      use.names = T, fill = T))
#
#                                            data.table::data.table(t(calculateStats(df = dftmp,
#                                                                        ds1 = x,
#                                                                        ds2 = unique(ref$dataset),
#                                                                        column = column,
#                                                                        subsampleSize = subsampleSize,
...
#     if (permutationPvalues) {
#         perm_stats <-
#             data.table::rbindlist(bplapply(setNames(rep(unique(df$dataset), nPermutations),
#                                                     rep(unique(df$dataset), nPermutations)),
#                                            function(x, ref, df,
#                                                     calculateStats,
#                                                     column,
#                                                     subsampleSize,
...
#                                   idcol = "dataset")
#
#         perm_stats <- rbindlist(list(obs_stats, perm_stats))
#     }
#
#
#
#             ## Permutation p-values
#             obs_stats["NNmismatchP"] <-
#                 signif(mean(c(obs_stats["NNmismatch"], perm_stats[, "NNmismatch"]) >= obs_stats["NNmismatch"]),
#                        3)
#             obs_stats["avesilhP"] <-
#                 signif(mean(c(obs_stats["avesilh"],
#                               perm_stats[, "avesilh"]) >= obs_stats["avesilh"]),
#                        3)
#             obs_stats["avesilhlocalP"] <-
#                 signif(mean(c(obs_stats["avesilhlocal"],
#                               perm_stats[, "avesilhlocal"]) >=  obs_stats["avesilhlocal"]),
#                        3)
#             if (length(column) == 1) {
#                 obs_stats["diffareaP"] <-
#                     signif(mean(c(obs_stats["diffarea"],
#                                   perm_stats[, "diffarea"]) >= obs_stats["diffarea"]),
#                            3)
#             }
#         }
#         obs_stats
#
#
#     # ds_res <- data.frame(data.table::rbindlist(lapply(ds_res_l, function(x)data.table(t(x))), idcol = "dataset"))
#     ds_res
#
# }
